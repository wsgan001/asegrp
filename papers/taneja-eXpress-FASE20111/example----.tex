...
\begin{itemize}
\vspace{-1ex}
\item \textbf{Category $B_{E+I}$.} On traversing the CFG in Figure~\ref{fig:Tree}, \CodeIn{eXpress} detects that taking the branches $<$$2, 3$$>$\footnote{$<$$x,y$$>$ represents a branch from source vertex $x$ to destination vertex $y$}, $<$$4, 16$$>$, $<$$16, 17$$>$, $<$$16, 18$$>$, $<$$12, 13$$>$ and $<$$14, 15$$>$ (dotted edges in Figure~\ref{fig:Tree}), the program execution cannot reach any of the black vertices. Hence, the execution of these branches cannot help in executing the changed statements or infecting the program state. \\ 
\vspace{-1ex} 
...
\CodeIn{eXpress} instruments the new program version to guide test generation to achieve condition I of the PIE model.
In particular, \CodeIn{eXpress} invokes the changed regions from the two program versions (with same inputs) and 
...
Figure~\ref{fig:example}. For the changed regions (Lines 10 and 11) of the two versions, \CodeIn{eXpress} extracts the
changed regions in separate method bodies. The method \CodeIn{fOld} shows the method extracted for the old program version,
...
Since lines 10 and 11 on left side of Figure~\ref{fig:example} do not exist in the old program version, the method \CodeIn{fOld} just returns a current parameter $state$,
while the method \CodeIn{fNew} contains Lines 10 and 11 and returns the resulting value of $state$.
\CodeIn{TestMe} invokes the methods \CodeIn{fOld} and \CodeIn{fNew} (Lines 11 and 12). 
Note that the parameter $stateOld$ passed to 
\CodeIn{fOld} is a copy of the parameter $state$ passed to \CodeIn{fNew} (Line 10), since $state$ is an output of $fNew$, hence it may be updated by the invocation to $fNew$. 
For the method corresponding to the old changed region, the parameters are the copies of the original.
\CodeIn{eXpress} then inserts branches in the program representing outputs of the two methods.
If the outputs of the two methods are different if and only if the program state is infected.
During test generation, \CodeIn{eXpress} tries to flip these inserted branches to infect the program state.
\begin{figure}[t]
    \centering
        %\includegraphics[width=6cm, keepaspectratio]{Figures/cfg3}
         \subfigure{\includegraphics[width=4.5cm, keepaspectratio]{Figures/cfg}}
         \hspace{2cm}
         \subfigure{\includegraphics[width=4.5cm, keepaspectratio]{Figures/ExecutionTreeEPS}}
    \vspace{-0.4cm}
...
\\ \textbf{Test Generation. }To cover the changed statements at Line 13 and infect the program state, 
 DSE needs at least 6 DSE runs (starting from an empty input array $c$). However, the number of runs depends on the choice of branches that DSE flips in each run. In each run, DSE has the choice of flipping 8 branches in the program. For the program in Figure~\ref{fig:example}, Pex takes 441 DSE runs to cover the true branch of statement at Line 13 (to infect the program state). The number of runs can be much more if the number of branching statements in the program increases. To reduce the search space of DSE, \CodeIn{eXpress} prunes branches in category 
