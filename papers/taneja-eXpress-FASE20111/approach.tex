...
\CodeIn{eXpress} synthesizes a class $C_o$, such that 
...
In each method $M_{\delta_{i1}}$ and $M_{\delta_{i2}}$, after the instructions in $\delta1$ $\delta$,
\CodeIn{eXpress} instantiates an object $c_o$ of $C_o$ and assigns the resulting value of each variable $o_{i1}$ in $M_{\delta_{i1}}$ (or $o_{i2}$ in $M_{\delta_{i2}}$) to $c_o.o_i$. 
...
\CodeIn{eXpress} inserts the methods in the new program version.
