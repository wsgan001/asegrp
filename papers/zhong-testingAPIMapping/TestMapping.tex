...
\begin{abstract}
Application Programming Interface (API) mapping relations describe the relation between an API invocation of one language and its mapped API invocations in another language. With API mapping relations, migration tools can translate API client code from one language into another language. Still, some mapped API invocations have different behaviors, and they produce different outputs given the same inputs. These mapped API invocations introduce defects in translated code silently since they do not introduce compilation errors. It is desirable to detect these differences for improvement of migration tools, but it is challenging to detect such differences since it requires comparing various behaviors of mapped invocations. In this paper, we propose an approach, called TeMaAPI (\textbf{Te}sting \textbf{Ma}pping relations of \textbf{API}s), that detects different behaviors of mapped API invocations via testing. In particular, TeMaAPI generates adequate test cases that can visit all feasible paths and invocation sequences of each API method. Based on our approach, we implement a tool and conduct evaluations on 5 migration tools. The results show that TeMaAPI effectively detects many different behaviors of mapped API invocations. We further analyze the detected differences, and conclude 9 findings and 8 implications. 
These findings and implications may help improve existing migration tools and are also helpful for programmers to understand the differences of API invocations between Java and C\#.
