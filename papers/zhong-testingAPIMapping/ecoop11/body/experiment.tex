...
We find that two mapped API methods can throw exceptions that are not mapped. For example, the \CodeIn{java.lang.StringBuffer. insert(int,char)} method in Java throws \CodeIn{ArrayIndexOut\-of\-BoundsException} when indexes are out of bounds, and Java2CSharp translates the method to the \CodeIn{System.Text.StringBuilder. Insert(int,char)} method in C\# that throws \CodeIn{ArgumentOut\-Of\-RangeException} when indexes are out of bounds. As Java2CSharp translates \CodeIn{ArrayIndexOutofBoundsExcep\-tion} in Java to \CodeIn{Index\-Out\-Of\-RangeException} in C\#, the mapped C\# method fails to catch exceptions when indexes are out of bounds.
