...
\item How effectively can our approach detect different behaviors of multiple API invocations (Section~\ref{sec:evaluation:sequence})?
...
In this example, Java allows programmers to access a stream even if the stream is closed, so the preceding JUnit test case run successfully. C\# does not allow such accesses, so the preceding NUnit test case fails with \CodeIn{ObjectDisposedException}.
