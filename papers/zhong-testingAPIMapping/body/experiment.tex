...
To translate one API invocation from Java to C\#, JLCA adds the \CodeIn{tmp\_Control} local variable and generates the \CodeIn{SupprtClass} class where more API invocations are introduced.
...
\hline
XmlSAXDocumentManager          &  10\%  & 17\%  &  18\%\\
\hline
...
\hline
\end{tabular}\vspace*{-2ex}
\Caption{Results of testing coverage} \label{table:coverage}
\end{SmallOut}\vspace*{-2ex}
\end{table}
Test coverage is a common criterion to measure the adequacy of test cases~\cite{zhu1997software}. To investigate whether our combination strategy helps achieve better coverage, we conduct an additional experiment on several selected classes, and Table~\ref{table:coverage} shows the results. Column ``Class'' shows names of classes. We select only several classes as subjects since it is quite trick to extract coverage of API methods, especially in our context where most mapping relations are between J2SE and \CodeIn{mscorlib.dll}. For example, coverage tools such as PartCover\footnote{\url{http://partcover.blogspot.com/ }} rely on the \CodeIn{JITCompilationStarted} method\footnote{\url{http://tinyurl.com/2gy2nqk}} for notifications of called methods, and thus fail to extract coverage info for many methods in \CodeIn{mscorlib.dll} since usually no notifications are received when these method are called.
