...
\begin {tabular} {|c|r|r|r|r|r|c|c|}
 \hline
\multirow{2}*[-2pt]{\textbf{Name}}
& \multirow{2}*[-2pt]{\textbf{NUnit}} & \multicolumn{2}{|c|}{\textbf{Failure}} \\\cline{3-4}  &  & \textbf{M}& \textbf{\%} \\
\hline
...
Java allows programmers to access a stream even if the stream is closed, so the preceding JUnit test case run successfully. C\# does not allow such accesses, so the preceding NUnit test case fails with \CodeIn{ObjectDisposedException}.
