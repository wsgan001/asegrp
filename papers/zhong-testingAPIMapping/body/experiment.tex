...
\textbf{Implication 1:} Although programmers may come to agreements on functionalities of API invocations, the behaviors for null values are typically controversial. Programmers should check whether different behaviors of null values will affect the behaviors of project under development. It is also desirable that a migration tool can hide the differences by analyzing the behaviors of mapped API methods given null values.
...
Each Java class has a \CodeIn{toString()} method inherited from the \CodeIn{java.lang.Object} class, and each C\# class also has a \CodeIn{ToString()} method inherited from the \CodeIn{System.Object} class. 
