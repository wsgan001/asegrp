...
When TeMAPI synthesizes wrapper methods for non-static fields or methods, it uses translatable constructors. The reason is that a translation tool may not be able to translate all constructors, resulting in wrapper methods that cannot be compiled. Therefore, to address this issue, we identify translatable constructors and use them with non-static fields or methods.
...
TeMAPI groups all synthesized wrapper methods for one API class $C$ to one synthesized class. When synthesizing, TeMAPI ignores generic methods, since the translation tools cannot handle generics. Furthermore, when synthesizing wrappers for C\# API classes, it ignores \CodeIn{unsafe} methods, \CodeIn{delegate} methods, and methods whose parameters are marked as \CodeIn{out} or \CodeIn{ref} besides generic methods. Java does not have corresponding keywords, so existing translation tools typically do not translate the preceding methods. After wrappers are synthesized, TeMAPI uses the translation tool under analysis to translate wrappers to the other language.
