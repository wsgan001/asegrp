...
example accepts a \CodeIn{string} input that represents name of a
...
\CodeIn{file}, of type \CodeIn{java.io.File} and calls the
\CodeIn{exists} method. Here, we consider \CodeIn{file} as a
receiver for the \CodeIn{exists} method.
...
call the \CodeIn{Exist} method and the \CodeIn{FullName} method to
...
the API class \CodeIn{java.io.File} of Java to
\CodeIn{System.IO.FileInfo} of C\#.
...
API method \CodeIn{Exist} from one language to another.
