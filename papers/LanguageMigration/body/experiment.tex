...
To investigate the first research question, we use the first 10 projects
...
In summary, as shown by Row ``Average'', our approach aligns classes and methods well on average. The result confirms that many
...
\textbf{Comparing with manually written API mapping relations.} Some migration
...
\hline
\end{tabular}\vspace*{-2ex}
\Caption{Results of comparison with manually written relations} \label{table:compare}
\end{SmallOut}\vspace*{-4ex}
\end{table}

This item describes the mapping relation between \CodeIn{java.math. BigDecimal} of Java and \CodeIn{System.Decimal} of C\#, and also describes the mapping relation between \CodeIn{multiply}
of Java and \CodeIn{Multiply} of C\#. For the two API methods, the pattern
...
\CodeIn{multiply} method. Based on this item, Java2CSharp migrates
...
\CodeIn{System.Xml.XmlAttribute}, and the mapping files of Java2CSharp do not contain mapping relations for the two API classes. If we consider these 25 new relations as true positives,
...
by \CodeIn{getTime} of Java) is equal to 1000 ticks (returned by \CodeIn{Ticks} of C\#).
...
\hline
\end{tabular}\vspace*{-2ex}
\Caption{\label{table:errors}Compilation errors and defects (SimM: SimMetrics)}
\end{SmallOut}\vspace*{-5ex}
\end{table}
...
This statement contains an API related defect since Java2CSharp does not migrate the \CodeIn{sqrt} method of Java to its corresponding API method of C\#. Sub-columns ``\emph{\%E}'' and ``\emph{\%D}'' list
...
Neethi\footnote{\url{http://ws.apache.org/commons/neethi/}}\Comment{ and
