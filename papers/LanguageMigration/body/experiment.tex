...
Column ``Ext. MF'' lists results with mapping files that combine our
mined API mapping relations with the existing mapping files of
Java2CSharp. Sub-columns ``\emph{E}'' and ``\emph{D}'' list number
of compilation errors and found defects, respectively. For each
...
files by comparing existing C\# files for API related defects.
Sub-columns ``\emph{\%E}'' and ``\emph{\%D}'' list percentages of
improvements over the results of ``MF''. On average, mined API
mapping relations help further reduce 54.4\% compilation errors and
43.0\% found defects. As the five projects use different libraries,
...
\emph{simmetrics} and \emph{fit} have a few errors and defects. The
...
buggy translated GUI files and we do not find any defects. The mined
...
both many errors and defects. In particular, \emph{llrp} uses
...
the mined API mapping helps reduce compilation errors and defects
...
mapping helps effectively reduce compilation errors and defects in
...
determining correct mined API mapping and for determining defects in
translated projects. To reduce the threats, we inspected mined
mapping relations and defects carefully, and we referred to existing
mapping relations and existing C\# files for determining correct API
mapping and defects, respectively. The former threat could be
...
cases when we improve our approach and produce translated projects
