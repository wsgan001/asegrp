...
maps 51 API classes of these packages to its implemented classes to
hide different behaviors from client code. For example, Java2CSharp
maps \CodeIn{java.util.Set} to \CodeIn{ILOG.J2CsMapping.
Collections.ISet}. Our approach did not mine these mapping relations
since the subjects in Table~\ref{table:subjects} do not use
Java2CSharp's own implemented classes and methods. This impact
factor reduces both the precisions and the recalls.
...
files of Java2CSharp cover only 1 class of \CodeIn{java.awt}, so the
...
map \CodeIn{java.awt} to \CodeIn{System.Windows.Forms} and thus
