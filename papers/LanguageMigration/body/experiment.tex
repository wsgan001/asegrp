...
manually inspect top 30 mined API mapping relations and classify
...
mined mapping files. If we consider these 25 new relations as true positives,
the total precision would be xx\%. (@Hao, could you new value?)
For example, these mined mapping files contain
a mapping relation between \CodeIn{org.w3c.dom.Attr} and
\CodeIn{System.Xml.XmlAttribute}, and the mapping relation does not
exist in the mapping files of Java2CSharp. These new mapping relations are useful and
...
Sub-columns ``\emph{E}'' and ``\emph{B}'' list the number of
compilation errors and found defects, respectively. For each project, 
we select those overlapping files between translated files and existing C\# files 
from the C\# version of the five projects, and manually inspect
the top five largest files among these overlapping files for detecting API related defects. (@Hao: Could
you please give some examples of API related defects)
Sub-columns ``\emph{\%E}'' and ``\emph{\%B}'' list
