...
This item describes mapping relations between the \CodeIn{multiply}
...
negatives represent those mapping relations that exist in the golden
standard but not in the mined API mapping. As shown in
...
three main reasons. First, we find 25 correct mapping
...
exist in the mapping files of Java2CSharp. As these relations are
considered as false positives, these relations reduced our
precision values. Second, although we use 10 large projects as subjects to
mine API mapping, these projects do not cover mapping relations of
all API classes and methods. Consequently, our approach does
not mine mapping relations of the entire API classes and 
methods. As shown in Table~\ref{table:minedresults}, our
approach mines many mapping relations and these mapping relations cover
many libraries. However, when we limit mapping relations to the packages as
shown in Table~\ref{table:compare}, these mined mapping relations are
actually not many as expected. In contrast, the mapping files
of Java2CSharp are more detailed as they are manually built. These
factors reduced our recall values. Third, we find that Java2CSharp
...
classes and methods. This factor reduced both the precision
...
mapping relations are accurate and include new mapping relations
...
to achieve more detailed API mapping.
\begin{table}[t]
\centering
\begin{SmallOut}
