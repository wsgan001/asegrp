...
\textbf{API transformation graph.} An API transformation graph (ATG)
...
multiply(BigDecimal)} has a related variable ($v_1$) and a parameter
...
%related variable and the parameters of a method as the inputs of a
...
%\CodeIn{java.io. File.exist()} has a related variable whereas
%\CodeIn{System.IO.File.Exist()} has no related variable but a
...
For the two source files shown in Section~\ref{sec:example},
Figure~\ref{fig:graph} (a) shows the partial ATG of
\CodeIn{IndexFiles.cs}, and Figure~\ref{fig:graph} (b) shows the
partial ATG of \CodeIn{IndexFiles.java}. Figure~\ref{fig:graph} also
...
named as \CodeIn{filename} to the parameter named as \CodeIn{arg0}
