...
(@Hao: Could you please change the image to Algorithm 1 from Algorithm 2)
...
to mine mapping relations of API methods. First, the 
\CodeIn{findVarPairs} function finds mapped variables $\in$ $F$, $V$, and $P_1$ in $m$ and $m'$. 
\CodeIn{findVarPairs} maps two variables $v$ and $v'$ of methods $m$ and $m'$,
...
cannot be mapped to the return of \CodeIn{java.io.File.File()}. Therefore, our
approach merges subsequent API methods in the ATG till the \CodeIn{Exists}
