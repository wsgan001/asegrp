...
major challenges. First, mapping inputs of an API method in one language with
inputs of an API method in the other language can be complex. Second, an API
...
to the built ATG (and sub-graphs inside ATG) and these additional edges
...
\item $\forall$ statements in the form $x = AM()$, where $x \in F \cup V$, MAM
...
\item $\forall$ statements in the form $m_2(m_1(x))$, MAM
...
\item $\forall$ statements in the form $x.m()$, MAM adds
...
cannot be mapped to the return of \CodeIn{java.io.File.File()}. Therefore, 
MAM merges subsequent API methods in the ATG till the \CodeIn{Exists}
