...
namespace NUnit.Util.PUTs
{
    [TestFixture]
	[PexClass]
    public partial class PathUtilTests : PathUtils
	{
		[PexMethod]
    ...
    public partial class PathUtilTests_Windows : PathUtils
	{
		[TestInitialize]
        ...
        [PexMethod]
        public void PathFromUriPUT1([PexAssumeUnderTest]String path)
        {
            PexAssume.IsTrue(path.EndsWith(".dll"));
            if (path.StartsWith("file:///"))
            {
                String filteredPath = PathUtils.GetAssemblyPathFromFileUri(path);
                String oraclePath = path.Substring(8, path.Length - 8);
                Console.WriteLine(oraclePath);
                PexAssert.AreEqual(filteredPath, oraclePath);
            }
            else if (path.StartsWith("file://"))
            {
                String filteredPath = PathUtils.GetAssemblyPathFromFileUri(path);
                String oraclePath = path.Substring(7, path.Length - 7);
                PexAssert.AreEqual(filteredPath, oraclePath);
            }
        }
	}
