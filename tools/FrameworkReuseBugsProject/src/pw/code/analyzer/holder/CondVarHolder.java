...
public class CondVarHolder implements Comparator{
	...
	public boolean equals(Object otherObj)
	{
		...
		//Incase of Null check and equality check, make an additional comparison on operator also
		//This can be wrong, as many places it is done with "==" and many others it can be done with "!="
		//So currently this is commented out.
		/*if(condType == NULL_CHECK || condType == METHODARG_NULL_CHECK || condType == CONSTANT_EQUALITY_CHECK
				|| condType == METHODARG_CONSTANT_EQUALITY_CHECK) {
			if(this.operator == null || cvh.operator == null) {
				logger.error("ATTENTION: One or both operators in comparison are empty");
			} else {
				if(!this.operator.equals(cvh.operator)) {
					return false;
				}
			}
		}*/
	...
	//PS: This has to be invoked only after favorable and total are set.
	public void setSupport(double support) {
		
		if(support == 1.0 && totalRelatedFrequency <= CommonConstants.PENALTY_THRESHOLD) {
			support -= CommonConstants.PENALTY_FOR_LOW_SAMPLES;			
		}
