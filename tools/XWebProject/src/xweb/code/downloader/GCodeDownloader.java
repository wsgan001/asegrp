...
public class GCodeDownloader {
			    ...
			    for(int cnt = 0; cnt < cseObjArr.length; cnt ++)
		    	{
		    		if(cseObjArr[cnt] instanceof CodeSearchEntry)
		    		{
		    			...
		    			{	
					    	...
					    	{
					    		addToLists(cseObj, URLLinkList, FileNameList);
					    	}
					    	else
					    	{	
						    	for(CodeSearchEntry subcseObj : sub_entries)
						    	{
						    		addToLists(subcseObj, URLLinkList, FileNameList);
						    	}
					    	}	
		    			}
		    			else
	...
	private void addToLists(CodeSearchEntry cseObj, List<String> URLLinkList, List<String> FileNameList)
	{
		...
		try
		{
			...
			fileNameList.add(fileName + "." + language);
