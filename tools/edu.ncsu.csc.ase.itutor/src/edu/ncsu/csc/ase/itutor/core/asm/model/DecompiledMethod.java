...
import edu.ncsu.csc.ase.itutor.core.asm.visitors.CommentedClassVisitor;
...
public class DecompiledMethod {
	...
	public DecompiledMethod(final String owner, final List inputText, final Map lineNumbers, final MethodNode meth,
			final ClassLoader cl, BitSet modes) {
		this.owner = owner;
		...
		if (modes.get(BCOConstants.F_SHOW_ANALYZER) && (meth.access & Opcodes.ACC_ABSTRACT) == 0) {
			analyzeMethod(cl);
		}
	}

	public boolean isInit() {
		return ("<init>".equals(meth.name) && "()V".equals(meth.desc)) || "<clinit>".equals(meth.name);
	}
	...
	/**
	 * @param sourceLine
	 * @return nearest match above given source line or the given line for perfect match or -1 for no match. The return value is
	 *         method-relative, and need to be transformed to class absolute
	 */
	...
	private void formatText(final List input, final Map locals, StringBuffer line, final List result) {
	 ...
	 * @return array with two elements, first is the local variables table, second is the operands stack content. "null" value could
	 *         be returned too.
	 */
	public String[] getFrame(final int decompiledLine, final boolean useQualifiedNames) {
		...
		if (frames != null && insn != null) {
			...
			try {
				...
				// TODO should we keep this?
				//UnitPlusPlugin.logWarningMessage(e.getMessage());
			}
		}
		return null;
	}
	...
	public String[][][] getFrameTables(final int decompiledLine, boolean useQualifiedNames) {
		...
		if (frames != null && insn != null && insn.intValue() >= 0 && insn.intValue() < frames.length) {
			Frame f = frames[insn.intValue()];
			if (f == null) {
				return null;
			}
			...
			try {
				...
				for (int i = 0; i < f.getLocals(); ++i) {
					...
					locals.add(new String[] { "" + i, getTypeName(useQualifiedNames, f.getLocal(i).toString()), varName });
				}

				ArrayList stack = new ArrayList();
				for (int i = 0; i < f.getStackSize(); ++i) {
					stack.add(new String[] { "" + i, getTypeName(useQualifiedNames, f.getStack(i).toString()) });
				}
				return new String[][][] { (String[][]) locals.toArray(new String[3][]), (String[][]) stack.toArray(new String[2][]) };
			} catch (IndexOutOfBoundsException e) {
				// TODO should we keep this?
				e.printStackTrace();
			}
	 ...
	 *            if false, then e.g. "Object" will be appended to buffer instead of "Ljava/lang/Object;" etc
	 * @param buf
	 *            buffer to append
	 * @param s
	 *            string with bytecode type name, like "Ljava/lang/Object;"
	 */
