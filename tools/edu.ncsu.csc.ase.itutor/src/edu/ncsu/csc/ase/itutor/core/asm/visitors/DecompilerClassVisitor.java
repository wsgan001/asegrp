package edu.ncsu.csc.ase.itutor.core.asm.visitors;
...
import edu.ncsu.csc.ase.itutor.core.asm.BCOConstants;
import edu.ncsu.csc.ase.itutor.core.asm.model.DecompiledClass;
import edu.ncsu.csc.ase.itutor.core.asm.visitors.CommentedClassVisitor.CommentedAnnotationVisitor;
import edu.ncsu.csc.ase.itutor.core.asm.visitors.CommentedClassVisitor.CommentedFieldVisitor;
...
public class DecompilerClassVisitor extends ClassAdapter {
	...
	public DecompilerClassVisitor(final ClassVisitor cv, final String field, final String method, final BitSet modes) {
		super(cv);
	...
	public static DecompiledClass getDecompiledClass(final InputStream is, final String field, final String method,
		final BitSet modes, final ClassLoader cl) throws IOException, UnsupportedClassVersionError {
...
//		}
		DecompilerClassVisitor dcv = new DecompilerClassVisitor(cv, field, method, modes);
	...
	private void formatText(final List input, StringBuffer line, final List result, final ClassLoader cl) {
	...
	public void visit(final int version, final int access, final String name1, final String signature, final String superName,
	...
	public void visitOuterClass(final String owner, final String name1, final String desc) {
		if (methodFilter == null && fieldFilter == null) {
	...
	public void visitInnerClass(final String name1, final String outerName, final String innerName, final int access) {
		if (methodFilter == null && fieldFilter == null) {
	...
	public FieldVisitor visitField(final int access, final String name1, final String desc, final String signature,
		final Object value) {
	...
	public MethodVisitor visitMethod(final int access, final String name1, final String desc, final String signature,
		...
		MethodNode meth = (MethodNode) classNode.visitMethod(access, name1, desc, signature, exceptions);
		List text = ((AbstractVisitor) cv).getText();
		int size = text.size();
		MethodVisitor mv = cv.visitMethod(access, name1, desc, signature, exceptions);
